//ParaviaInterface is used to define functions used for the background
//functions used for the ingame interactions while the game is looping
...
void ParaviaHelp::InitializePlayer(player *Me, int year, int city, int level, char *name, bool Morf)
{
	Me->Cathedral = 0;
...
bool ParaviaHelp::CheckNewTitle(player *Me)
{
	int Total;
...
void ParaviaHelp::GenerateHarvest(player *Me)
{
...
void ParaviaHelp::GenerateIncome(player *Me)
{
	float y;
	int revenues = 0;
	char str[256];
	...
	switch(Me->Justice)
	{
	    case 1: strcpy(str, "Very Fair"); 
	    	break;
	    case 2: strcpy(str, "Moderate"); 
	    	break;
	    case 3: strcpy(str, "Harsh"); 
	    	break;
	    case 4: strcpy(str, "Outrageous");
	}
...
void ParaviaHelp::ChangeTitle(player *Me)
{
	if(Me->MaleOrFemale == true)
...
void ParaviaHelp::NewLandAndGrainPrices(player *Me)
{
	float x, y, MyRandom;
	int h;
...
int ParaviaHelp::ReleaseGrain(player *Me)
{
	double xp, zp;
	float x, z;
	char string[256];
	...
	while(IsOK == false)
	{
		...
		cout << "1 - Minimun " << Minimum << ", 2 - Maximum " << Maximum << ", or enter a value: ";
...
void ParaviaHelp::SeizeAssets(player *Me)
{
	char string[256];
	Me->Marketplaces = 0;
	Me->Palace = 0;
...
void ParaviaHelp::SellGrain(player *Me)
{
	char string[256];
	int HowMuch;
...
void ParaviaHelp::SellLand(player *Me)
{
	char string[256];
	int HowMuch;
